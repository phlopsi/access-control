 ...
 *
 * @method     ChildPermissionQuery leftJoinPermissionToRole($relationAlias = null) Adds a LEFT JOIN clause to the query using the PermissionToRole relation
 * @method     ChildPermissionQuery rightJoinPermissionToRole($relationAlias = null) Adds a RIGHT JOIN clause to the query using the PermissionToRole relation
 * @method     ChildPermissionQuery innerJoinPermissionToRole($relationAlias = null) Adds a INNER JOIN clause to the query using the PermissionToRole relation
 *
 * @method     \Phlopsi\AccessControl\Propel\PermissionToRoleQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PermissionQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'access_control', $modelName = '\\Phlopsi\\AccessControl\\Propel\\Permission', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT external_id, id FROM permissions WHERE id = :p0';
        try {
    ...
    public function filterByPermissionToRole($permissionToRole, $comparison = null)
    {
        if ($permissionToRole instanceof \Phlopsi\AccessControl\Propel\PermissionToRole) {
            return $this
                ->addUsingAlias(PermissionTableMap::COL_ID, $permissionToRole->getPermissionId(), $comparison);
